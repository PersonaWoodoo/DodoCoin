...
from database import get_emoji, update_balance
...
def save_referral(invitee_id: int, inviter_id: int):
...
async def cmd_start_ref(message: Message):
    ...
    if written:
        update_balance(inviter_id, REF_REWARD)
        ...
        try:
            await message.bot.send_message(
                ...
                text=(
                    ...
                    f"Начислено вам <b>{REF_REWARD:,}</b> {emoji}".replace(",", "\u202f")
                ),
                parse_mode="HTML"
            )
        except Exception:
...
async def cmd_ref(message: Message):
    ...
    text = (
        ...
        f"За каждого приглашённого: <b>+{REF_REWARD:,}</b> {emoji}".replace(",", "\u202f")
    )
