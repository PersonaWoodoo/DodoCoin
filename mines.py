...
async def legacy_mines_cb(query: CallbackQuery):
    ...
    async with _game_lock(query.from_user.id):
        ...
        if action == "collect":
            ...
            balance = await finalize_bet(query.from_user.id, float(bet), float(payout), "mines", f"collect_{opened}")
            NMINES_GAMES.pop(gid, None)
        ...
        if action == "cell":
            ...
            if idx in game["mines"]:
                ...
                for m in game["mines"]:
                    if game["field"][m] == "❔":
                        game["field"][m] = "💣"
                balance = await finalize_bet(query.from_user.id, float(bet), 0.0, "mines", "explode")
                NMINES_GAMES.pop(gid, None)
            ...
            if len(game["opened"]) >= safe_needed:
                ...
                payout = int(round(bet * coef))
                balance = await finalize_bet(query.from_user.id, float(bet), float(payout), "mines", "all_safe")
                NMINES_GAMES.pop(gid, None)
