...
import database
from games.config import BANK_TERMS
from games.utils import fmt_money, parse_bet, now_ts, update_balance, reserve_bet
...
async def bank_term(call: CallbackQuery, state: FSMContext):
    ...
    ok, _ = await reserve_bet(call.from_user.id, amount)
    if not ok:
